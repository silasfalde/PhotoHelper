...
def add_panorama_subcommand(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory (default: repository root/current working dir)",
    )
...
def add_find_raws_subcommand(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.add_argument("--timeout", type=int, default=30, help="Timeout in seconds for downloading offloaded files (default: 30)")
    parser.set_defaults(handler=run_find_raws)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="One CLI for framing, collages, panoramas, and raw matching.")
